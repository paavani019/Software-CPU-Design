...
void cpu_decode_execute(CPU *cpu, uint16_t instruction, bool trace)
{
    ...
    case OP_BRANCH:
    {
        ...
        case BRANCH_EQ:
            ...
            if (trace)
                printf("    BEQ 0x%04X (Z=%d)\n", addr, cpu->flags.Z);
            break;
        case BRANCH_NE:
            should_branch = !cpu->flags.Z;
            if (trace)
                printf("    BNE 0x%04X (Z=%d)\n", addr, cpu->flags.Z);
            break;
        case BRANCH_GT:
            should_branch = !cpu->flags.N && !cpu->flags.Z;
            if (trace)
                printf("    BGT 0x%04X (N=%d,Z=%d)\n", addr, cpu->flags.N, cpu->flags.Z);
            break;
        case BRANCH_LT:
            should_branch = cpu->flags.N;
            if (trace)
                printf("    BLT 0x%04X (N=%d)\n", addr, cpu->flags.N);
            break;
        case BRANCH_GE:
            should_branch = !cpu->flags.N;
            if (trace)
                printf("    BGE 0x%04X (N=%d)\n", addr, cpu->flags.N);
            break;
        case BRANCH_LE:
            should_branch = cpu->flags.N || cpu->flags.Z;
            if (trace)
                printf("    BLE 0x%04X (N=%d,Z=%d)\n", addr, cpu->flags.N, cpu->flags.Z);
            break;
        case BRANCH_CS:
            should_branch = cpu->flags.C;
            if (trace)
                printf("    BCS 0x%04X (C=%d)\n", addr, cpu->flags.C);
            break;
        case BRANCH_CC:
            should_branch = !cpu->flags.C;
            if (trace)
                printf("    BCC 0x%04X (C=%d)\n", addr, cpu->flags.C);
            break;
        }
    ...
    case OP_CALL:
        ...
        cpu->pc = addr;
        if (trace)
            printf("    CALL 0x%04X (return addr=0x%04X)\n", addr, cpu->pc);
        break;
